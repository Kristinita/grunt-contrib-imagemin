...
export default (grunt) => {
	grunt.registerMultiTask('imagemin', 'Minify PNG, JPEG, GIF and SVG images', function() {
		...
		const processFile = (file) => Promise.resolve(grunt.file.read(file.src[0], {
			encoding: null,
		})).then((buf) => Promise.all([imagemin.buffer(buf, {
			plugins,
		}), buf]))
			.then((res) => {
			// TODO: Use destructuring when targeting Node.js 6
				const [optimizedBuf] = res;
		...
		pMap(this.files, processFile, {
			...
			if (totalFiles > 0)
				msg += chalk.gray(` (saved ${prettyBytes(totalSavedBytes)} - ${percent
					.toFixed(1)
					.replace(/\.0$/, '')}%)`);
